<template>
  <div>
    <nav-header @rylogined="askLogin" :num="checkedCount"></nav-header>
              ...
              <li v-for="item in cartList" :key="item._id">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="editCart('delete', item)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="editCheckAll(checkAll)">
                  <span class="checkbox-btn item-check-btn" :class="{'check':checkAll=='1'}">
...
export default {
  ...
  methods:{
      getCartList(){ // 请求数据，渲染购物车页面
      /**
       * 1. 发送get请求，请求购物车中数据
       * 2. 添加功能，实时与数据库进行数据往来，实现商品选中、数量、总额的状态更新
       * 3. 添加功能，实时与数据库进行数据往来，过滤出购物车中已选中商品，实时更新选中商品的总价格
       * 4. 添加功能，通过全选按钮，实时控制和更新购物车中所有商品的选中情况
       */
        this.checkedList = []
        axios.get('/users/cartList').then(res=>{
            this.cartList = res.data.result
              ...
              this.checkedList.forEach(item=>{
                  allCheckedP += item.salePrice*item.productNum
                  allCheckedC += parseInt(item.productNum)
              })
              this.checkedPrice = allCheckedP
              this.checkedCount = allCheckedC
              
          }) 
      },
      editCart(option, obj){ // 修改购物车中的商品信息
      /**
       * 1. 请求服务器及数据库，实现购物车中商品选中情况、选购数量状态的修改
       * 2. 实现购物车中商品的额删除功能
       */
        if(option == 'delete'){
          axios.post('users/cartDel',{productId: obj.productId}).then(res=>{
      ...
      editCheckAll(option){
        /**
         * 1. 请求服务器与数据库，通过Select All按钮控制购物车中所有商品的选中情况，并刷新页面
         */
        this.checkAll = !option
