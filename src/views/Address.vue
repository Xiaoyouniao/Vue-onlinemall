<template>
  <div>
    <nav-header @rylogined="askLogin"></nav-header>
    <nav-bread>
      <span>Address</span>
    </nav-bread>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item, index) in addressListsFilter" :key="item._id" :class="{'check': checkIndex == index}" @click="checkIndex=index">
                    <dl>
                      <dt>{{ item.userName }}</dt>
                    ...
                    <div class="addr-opration addr-set-default" v-if="!item.isDefault">
                      <a href="javascript:;" class="addr-set-default-btn" @click="setDefault(item.addressId,index)"><i>Set default</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="item.isDefault">Default address</div>
                  </li>
                  <li class="addr-new" @click="addAddressFlag = true">
              ...
              <div class="shipping-addr-more">
                <a class="addr-more-btn up-down-btn" href="javascript:;" @click="toggleMore" :class="{'open': limit>3 }">
                  more
                  <i class="i-up-down">
            ...
            <div class="next-btn-wrap">
              <a class="btn btn--m btn--red" @click="confirmOrder">Next</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':addAddressFlag}">
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title">Add Address</div>
          <button class="md-close" @click="addAddressFlag=false">Close</button>
        </div>
          ...
          <div class="confirm-tips">
            ...
            <ul>
              <li class="regi_form_input">
                <input type="text" tabindex="1" name="userName" v-model="userName" class="regi_login_input regi_login_input_left" placeholder="Addressee" data-type="loginname">
              </li>
              <li class="regi_form_input noMargin">
                <input type="text" tabindex="2"  name="streetName" v-model="streetName" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="Addresss Detail">
              </li>
              <li class="regi_form_input noMargin">
                <input type="text" tabindex="3"  name="postCode" v-model="postCode" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="PostCode">
              </li>
              <li class="regi_form_input noMargin">
                <input type="text" tabindex="4"  name="tel" v-model="tel" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="Tel" @keyup.enter="addAddress">
              </li>
            </ul>
          </div>
    ...
    <div class="md-overlay" v-if="addAddressFlag" @click="addAddressFlag=false"></div>

    <modal :mdShow="mdShowFlag" @close="closeModal"> 
      <h3 slot="message">您目前尚未登录，请先登录</h3>
      <div slot="btnGroup">
...
export default {
  ...
  methods: {
    ...
    setDefault(id,index){
      axios.post('/users/setDefault', {addressId: id}).then(res=>{
        if(res.data.status==0){
          this.checkIndex = index
    ...
    addAddress(){
      if(!this.userName || !this.streetName || !this.postCode || !this.tel){
        ...
        axios.post('/users/addAddress',{
          ...
          if(res.data.status==0) this.getAddressLists()
          if(this.addressLists.length>this.limit){
            this.limit = this.addressLists.length
          }
          this.addAddressFlag = false
          this.userName = this.streetName = this.postCode =this.tel =''
        })
      }
    },
    confirmOrder(){
      this.$router.push({path:'/orderConfirm', query: {addressId: this.checkAddressId}})
    }
  },
  components: {
    NavHeader,
